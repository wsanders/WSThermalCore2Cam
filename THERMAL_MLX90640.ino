...
void setup() {
    ...
    // debug - draw a bounding rect behind sprite
    // (sprite is 126 x 94)
    M5.Lcd.fillRect(0,0,128,96,RED);
...
void battStat() {
    ...
    msg.drawRect(xorigin, yorigin,30,15, c);
    msg.drawRect(xorigin+29, yorigin+4,4,7,c);
    if (batPct > 25.0) {
        msg.fillRect(xorigin+2, yorigin+2,8,11,GREEN);
    }
    if (batPct > 50.0) {
        msg.fillRect(xorigin+11, yorigin+2,8,11,GREEN);
    }
    if (batPct > 75.0) {
        msg.fillRect(xorigin+20, yorigin+2,8,11,GREEN);
    }
}
